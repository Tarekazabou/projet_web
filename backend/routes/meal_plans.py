...
def get_meal_plan(plan_id):
    ...
    try:
        ...
        serialized = _serialize_meal_plan(plan_data, meal_plan.id, db)
        
        return success_response({'meal_plan': serialized})
        
    except Exception as e:
        logger.error(f"Error getting meal plan: {e}")
        return error_response('Failed to get meal plan', 500)
...
def update_meal_plan(plan_id):
    ...
    try:
        ...
        update_data = {}
        allowed_fields = ['planDate', 'mealType', 'servings', 'notes', 'mealName', 'calories', 'ingredients']
        ...
        if 'recipeId' in data:
            update_data['recipe'] = db.collection('Recipe').document(data['recipeId'])
        ...
        serialized = _serialize_meal_plan(updated_plan, plan_ref.id, db)
        ...
        return error_response('Failed to update meal plan', 500)
